...
A traditional software upgrade process is a tedious task which involves allocating service outages, executing the upgrade during low traffic hours, and rigorous manual testing after the upgrade to ensure the services are working properly. If an issue is discovered after the upgrade the system has to be reverted back to the previous version which aging could be a more daunting task than the upgrade.  

In NFV environment the software upgrade process is quite different. You have to deploy the new version as a new VNF instance, test and then migrate the traffic in incremental stages. Once the total service is migrated to the new instance the old instance can be shutdown. If the VNFs are designed as cloud native applications it should be possible to automate most of the activities involved in this.
...
## OpEx Saving
While some would say that NFV will provide OpEx savings that is somewhat long-term and yet to be proven.

In conclusion it should be noted that some of these benefits will not be available from day-1 and there may be additional work for yielding those. So it is important that Telcos consider NFV as a transformation journey and move forward employing proper strategies and tactics.
